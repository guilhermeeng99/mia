...
- **File / batch transcription** — recognizing an existing media file (with ffmpeg preprocessing, file output, per-run progress bar) is a deferred **Phase 5** feature that reuses Toolzy's cold path (`preprocess_to_wav`, `run_whisper`, `transcript_output_path`, `transcribe_audio`) **verbatim** — see [../REUSE-FROM-TOOLZY.md](../REUSE-FROM-TOOLZY.md). Not on the live path.
