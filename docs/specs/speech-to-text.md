...
7. **Silence gating (anti-hallucination, fixed)** — recognition **always** runs with **Silero VAD**, so only detected speech reaches Whisper; silence / non-speech cannot become invented text. Not user-tunable ([ADR-007](architecture.md#adr-007-on-demand-model-download--cpu-bundled--optional-cuda-engine)). The VAD is **tuned against mid-word cuts**: whisper.cpp's defaults (`min-silence 100 ms`, `pad 30 ms`) split segments on tiny intra-word energy dips and splice the halves. `server_args` therefore passes `--vad-min-silence-duration-ms 2000` + `--vad-speech-pad-ms 400` (faster-whisper's proven values): only a ≥2 s pause splits a segment and each segment keeps 400 ms of edge context, while long leading/trailing silence is still trimmed. Additionally — and this was the actual root cause of transcripts like "cons enso" / "agu enta" — whisper-server terminates every internal decode segment with `\n` in the JSON `text`, and a segment boundary can fall **mid-word** (`"ao cons\nenso…"`). Whisper's tokens carry word spacing themselves (a continuation segment starts without a leading space; a new word starts with one), so `parse_inference_text` **deletes** `\r`/`\n` instead of letting them collapse into spaces, which reconstructs the exact text. Both cargo-tested.
