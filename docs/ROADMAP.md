...
- ⬜ **Wire the dictionary bias prompt into warm Whisper** — `dictionary.rs` `build_bias_prompt` is implemented + cargo-tested, but its output is not yet fed as the warm-Whisper initial prompt. → [custom-dictionary.md](specs/custom-dictionary.md), [speech-to-text.md](specs/speech-to-text.md)
