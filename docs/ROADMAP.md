...
- 🚧 **Global PTT hotkey** — `global-hotkey` crate, push-to-talk that works while unfocused; press-and-hold + toggle modes. Pure core done + cargo-tested: accelerator parser/canonicalizer (round-trips, exact error messages), bare-key + reserved-chord guards, and the debounce + activation-mode `reduce()` reducer (Rules 3/4/9/10). `app/src-tauri/src/hotkey.rs`. Remaining (runtime): `GlobalHotKeyManager` registration + `WM_HOTKEY` event loop + the `register/update/get/check`/recorder commands + `Esc`-cancel + missing-release watchdog. → [hotkeys.md](specs/hotkeys.md) · [ADR-005](specs/architecture.md#adr-005-system-wide-text-injection)
- 🚧 **System tray + floating mic HUD** — `tray-icon` crate for the tray; a dark translucent always-on-top mic HUD with the listening → transcribing → inserting → error state machine and a live waveform/level meter. HUD Svelte component (`MicHud.svelte`: state machine + RMS-driven waveform, HUD tokens) built + build-verified. Remaining (runtime): the `tray-icon` tray, the transparent always-on-top HUD window plumbing (`hud.rs` + `tauri.conf` window) and the orchestrator event wiring that drives state/level. → [tray-and-hud.md](specs/tray-and-hud.md), [design-system.md](specs/design-system.md)
- 🚧 **Wire the end-to-end pipeline** — orchestrate hotkey → capture → VAD → warm STT → cleanup → inject, with HUD state transitions and cancel. Pure orchestrator core in `dictation.rs` cargo-tested: `next_phase` HUD state machine (illegal-signal no-op, cancel-from-any), `interpret_down`, `classify_cancel`, `build_result`. Remaining (runtime): the `start/stop/cancel_dictation` commands that drive real capture + warm STT + cleanup/dictionary/snippets + injection. → [dictation.md](specs/dictation.md) · [ADR-004](specs/architecture.md#adr-004-warm-resident-stt)
- ⬜ **pt-BR + English** — first-class language selection and the pt-BR-faithful Whisper path; remembered default language. → [speech-to-text.md](specs/speech-to-text.md) · [ADR-003](specs/architecture.md#adr-003-whisper-stt)
...
- ⬜ Decide the warm-STT default in practice: confirm **whisper-rs in-process** meets the latency target, otherwise fall back to **whisper-server** ([ADR-004](specs/architecture.md#adr-004-warm-resident-stt)).
